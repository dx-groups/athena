...
function start() {
  ...
  choosePort(config.dev.host, config.dev.port)
    .then((port) => {
      ...
      const mockFilePath = paths.resolveApp('.athena.mock.js')
      const exists = fs.existsSync(mockFilePath)
      let before = () => {}
      if (exists) {
        before = (app) => { apiMocker(app, mockFilePath) }
      } else {
        console.log(chalk.red('There is no `.athena.mock.js` file.'))
      }
